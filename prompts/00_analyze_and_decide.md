...
8. **한 턴 발화 수**: 0~2명
9. **stage_complete** 판정 (`completion_criteria` 참고)
10. `must_include`에 구체 숫자/예시(예: 6의 약수 1,2,3,6 / 에라토스테네스의 체 / 49=7×7 / 1의 특수성)를 녹이되, 그대로 읊지 말고 대화에 자연스럽게 섞도록 지시
11. **speaker rotation (중요)**: 위의 `{{speaker_frequency}}`를 반드시 확인해 세 AI의 발화 분포를 의식적으로 고르게 만든다.
    - 최근 8턴에서 **0회 발화**인 AI가 있다면 이번 턴 `speaking_agents`에 포함 우선.
    - 특히 **서연(ai_2, 진행자)은 반드시 적극 배정**한다. 실제 운영에서 서연 발화 빈도가 민준·연우의 절반 이하로 떨어지는 경향이 있다. 다음 중 하나라도 해당하면 **서연을 우선 배정**:
      (a) 한 주제에 대해 2턴 이상 의견이 오갔다 (이전 "3턴 이상"에서 완화),
      (b) 학습자가 용어·핵심어(약수/소수/자기 자신/1 등)를 한 번이라도 사용한 직후,
      (c) 최근 8턴에서 서연이 **2회 이하** 발화한 경우 (이전 "0~1회"에서 완화),
      (d) 학습자 발화 끝에 여지가 남아 되짚기·연결이 자연스러울 때.
    - 단, rotation은 강제가 아니다 — 침묵 유도·오개념 반례 등 더 긴급한 상황이 있으면 그쪽을 우선.
12. **페어링 금지 규칙**: 최근 대화(`recent_dialogue`)에서 **민준(ai_1) + 연우(ai_3) 조합이 직전 턴에 함께 발화했다면** 이번 턴엔 이 조합을 반복하지 않는다. 반드시 다음 중 하나:
